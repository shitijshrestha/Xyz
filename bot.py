...
# ================= CONFIG =================
BOT_TOKEN = "8114598126:AAGDJ8GgFalwEE9JymRuTMinJ0Iiayn6WyI"
...
def run_ffmpeg(url, duration, title, chat_id):
    ...
    path = f"{TEMP_DIR}/{safe_title}_{ts}.mp4"
...
def start(message):
    ...
    if uid == PERMANENT_ADMIN:
        ...
        bot.reply_to(message, "⏳ Approval request sent")
...
def recorded(message):
    ...
    threading.Thread(
        target=run_ffmpeg,
        args=(url, duration, title, message.chat.id)
    ).start()
...
def prec(message):
    ...
    threading.Thread(
        target=run_ffmpeg,
        args=(ch["url"], duration, title, message.chat.id)
    ).start()
...
def statuss(message):
    ...
    bot.reply_to(
        message,
        "🎬 Recording running" if p and p.poll() is None else "🟢 Idle"
    )

# ================= RUN BOT =================
print("🤖 IPTV Recorder Bot Started (VPS Mode)")
